...
export const ProductPageComponent: FC<Props> = ({ categories, productsWithCategories }) => {
  ...
  const productCreateUpdateHandler = async (data: CreateOrUpdateProductSchema) => {
    ...
    if (heroImage) {
      const imagePromise = Array.from(heroImage).map(async file => await uploadFile(file as File));
      try {
        const [heroImageUrl] = await Promise.all(imagePromise);
        if (images.length > 0) {
          const imagesPromises = Array.from(images).map(file => uploadFile(file));
          try {
            const imageUrls = (await Promise.all(imagesPromises)) as string[];
            switch (intent) {
              case 'create': {
                await createProduct({
